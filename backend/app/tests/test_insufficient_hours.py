...
class TestInsufficientHours:
    def test_exact_hours_sufficient(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="刚好够课时学员", phone="13800000100", remaining_hours=2)
        students[s.id] = s
    ...
    def test_zero_hours_student_cannot_book(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="零课时学员", phone="13800000101", remaining_hours=0)
        students[s.id] = s
    ...
    def test_fractional_hours_insufficient(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="部分课时学员", phone="13800000102", remaining_hours=1)
        students[s.id] = s
    ...
    def test_fractional_hours_sufficient(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="足够部分课时学员", phone="13800000103", remaining_hours=2)
        students[s.id] = s
    ...
    def test_multiple_bookings_exhaust_hours(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="多段预约学员", phone="13800000104", remaining_hours=3)
        students[s.id] = s
    ...
    def test_booking_duration_calculation_precise(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="精确计算学员", phone="13800000105", remaining_hours=1)
        students[s.id] = s
        ...
        payload = {
            ...
            "coach_id": test_coach.id,
            "start_time": future_slot.isoformat(),
            "end_time": (future_slot + timedelta(hours=1, minutes=1)).isoformat(),
        }
        response = client.post("/api/appointments", json=payload)
        assert response.status_code == 400

    def test_exact_one_minute_less_than_available(self, client, test_coach, future_slot):
        s = Student(id=next_id("student"), name="边界学员", phone="13800000106", remaining_hours=1)
        students[s.id] = s
